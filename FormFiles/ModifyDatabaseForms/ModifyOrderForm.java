...
/**
 * This class is used to be able to modify a user specified order to the database through the navigation of the
 * application
 *
 * @author Lucas Hynes
 * @version 1.1.0
...
public class ModifyOrderForm {
    ...
    @SuppressWarnings("unchecked") public Scene form(Order o) throws SQLException {
        ...
        Label cube_amount = new Label(String.valueOf(o.getCubes()));
        Label weight_amount = new Label(String.valueOf(o.getWeight()));
        Label piece_amount = new Label(String.valueOf(o.getPieces()));
        Label merch_price = new Label(String.valueOf(o.getMerch_price()));
        ...
        company_choice.getSelectionModel().select(o.getDistributor().getName());
        ...
        try {
            ...
            //sets the values for the selected order
            for(Node d : getPane().getChildren()) {
                if(GridPane.getRowIndex(d) != 0) {

                    switch (GridPane.getColumnIndex(d)) {
                        case 0:
                            //noinspection unchecked
                            ((ComboBox<String>) d).getSelectionModel().select(o.getStore_front().getName());
                            break;
                        case 1:
                            ((TextField) d).setText(String.valueOf(o.getCubes()));
                            break;
                        case 2:
                            ((TextField) d).setText(String.valueOf(o.getWeight()));
                            break;
                        case 3:
                            ((TextField) d).setText(String.valueOf(o.getPieces()));
                            break;
                        case 4:
                            ((TextField) d).setText(String.valueOf(o.getMerch_price()));
                            break;
                    }
                }
            }
        ...
        EventHandler<MouseEvent> save_event = event -> {
            ...
            try {
                ...
                commit(company_choice.getValue(), date.getValue(), o);
    ...
    @SuppressWarnings("unchecked") private void commit(String dist_name, LocalDate date, Order w) throws SQLException {
        ...
        for(int i = 0; i < getCounter() - 1; i++) {
            orders.add(new Order(w.getOrder_id(), "", "", d,
        ...
        for(Order o1: orders){
            ...
            if((o1.getStore_front() != null) && (o1.getDistributor() != null)) {

                //sets the rate through the distributor and store front ids'
                o1.setRate(getDba().findRate(o1.getDistributor().getDistributor_id(),
                        o1.getStore_front().getStore_front_id()));

                //applies the order to the database
                getDba().update_order(o1);
            }
        }
    }
