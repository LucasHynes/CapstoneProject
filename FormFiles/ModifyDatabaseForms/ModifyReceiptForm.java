 ...
 * @since 2/24/2021
 */
public class ModifyReceiptForm {
     ...
     */
    public Scene form(Receipt inr) throws SQLException {
        ...
        driver_choice.getSelectionModel().select(getDba().get_driver_based_id(inr.getDriver_id()).getDriver_name());
        ...
        company_choice.getSelectionModel().select(inr.getCategory());

        //holds the money amount
        TextField amount_field = new TextField();
        amount_field.setPromptText("No need for the $");
        amount_field.setText(String.valueOf(inr.getAmount()));

        //holds the card no used
        TextField card_no_field = new TextField();
        card_no_field.setPromptText("XXXX");
        card_no_field.setText(String.valueOf(inr.getCard_no()));

        //date for the receipt
        DatePicker datePicker = new DatePicker();

        //text fields for the location associated with the receipt
        TextField city_field = new TextField(inr.getCity());
        TextField state_field = new TextField(inr.getState());
        TextField zip_field = new TextField(String.valueOf(inr.getZip_code()));
        ...
        category_choice.getSelectionModel().select(inr.getCategory());
        ...
        EventHandler<MouseEvent> save_event = e -> {
            ...
            Receipt r = new Receipt(inr.getReceipt_id(), getDba().get_driver_id(driver_choice.getValue()),
        ...
        HBox button_group = new HBox(cancel_button, save_button);
